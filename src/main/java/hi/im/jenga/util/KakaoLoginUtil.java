...
import hi.im.jenga.member.dto.KakaoDTO;
...
public class KakaoLoginUtil {
    ...
    @Value("#{Kakao['KakaoJsKey']}")
    private String KAKAO_JS_KEY;

    @Value("#{Kakao['KakaoRestKey']}")
    ...
    public static JsonNode getAccessToken(String autorize_code) {
        ...
        postParams.add(new BasicNameValuePair("client_id", "KAKAO_REST_KEY")); // REST API KEY
        postParams.add(new BasicNameValuePair("redirect_uri", "http://localhost:8080/oauth")); // 리다이렉트 URI
  ...
  /*  public static KakaoDTO changeData(JsonNode userInfo) {
        KakaoDTO dto = new KakaoDTO();

        dto.setUser_snsId(userInfo.path("id").asText()); // id -> dto 넣기

        if (userInfo.path("kaccount_email_verified").asText().equals("true")) { // 이메일 받기 허용 한 경우
            dto.setUser_email(userInfo.path("kaccount_email").asText()); // email -> dto 넣기
